...
Один из основных моментов, что классический Superset настроен под "*классические*" БД и поэтому он предполагает
структуру:
...
И также теперь Superset корректно видит модель данных и схемы коннектора, потому что мы сделали подключение "*к
Database*" через Trino и для Superset это классическая база данных, где у нас есть `schema` -> `table`